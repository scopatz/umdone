...
class BaseAppModel(object):
    ...
    def __init__(
        self, audio, window_length=0.05, threshold=0.01, device=-1, dbfile=None
    ):
        self.dbfile = dbfile
        self.audio = (
            audio
            if isinstance(audio, sound.Audio)
            else sound.Audio.from_hash_or_init(audio)
        )
        # settings
        self.load_settings()
        ...
        if self.bounds is None:
            bounds = segment.boundaries(
                self.raw, self.sr, window_length=window_length, threshold=threshold
            )
    ...
    def save_settings(self):
        settings = {"device": self.device, "current_segments": self.current_segments}
...
class BaseAppView(urwid.WidgetWrap, urwid.PopUpLauncher):
    ...
    def update_status(self):
        ...
        if model.current_segment in model.categories:
            ...
            c = "Uncategorized"
        s = ("Clip {0} of {1}\n" "Duration {2:.3} sec\n" "{3}").format(
            model.current_segment + 1, model.nsegments, len(model.clip) / model.sr, c
        )
    ...
    def main_window(self):
        ...
        w = urwid.Columns(
            [("weight", 1, self.graph_wrap), ("fixed", 1, vline), (42, c)],
            dividechars=1,
            focus_column=2,
        )
...
class BaseAppDisplay(object):
    ...
    def main(self):
        ...
        self.loop.set_alarm_in(
            0.001, lambda w, d: self.select_segment(self.model.current_segment)
        )
        self.loop.run()
