...
class ClipperModel(BaseAppModel):
    ...
    def __init__(self, audio, dbfile=None, **kwargs):
        if dbfile is not None and os.path.isfile(dbfile):
            _, self.bounds, _ = umdone.io.load_clips(dbfile, raw=False, categories=False)
...
def main(ns=None, args=None):
    ...
    ClipperDisplay(ns.input, ns.output, window_length=ns.window_length,
                   noise_threshold=ns.noise_threshold).main()
